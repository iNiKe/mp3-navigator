...
const appName  = 'MP3 NaviGatoR';
      appVer   = 'v2.7';
      appBuildDate = '30/03/2002'; {DD/MM/YYYY}
...
*)
    ...
    procedure PlayListPTCustomDraw(aSender: TObject; aCD: TPTCustomDraw;
      aItem: TListItem);
...
{$I-}
  ...
  begin
    ...
    begin
      ...
      begin
        MessageDlg('Невозможно сохранить плейлист (в ReadOnly файл)!',mtError,[mbok],0);
        exit;
      end else DoSet := true;
    end;
  end;
...
begin
  ...
  Result := ExtractFilePath(Result) + s;
end;
...
(*
procedure tMainForm.WMPaint(var m: TWMPaint);
begin
  Inherited;{}
(*
var
...
begin
  ...
  if Assigned(StatusForm) then StatusForm.PathEdit.text := Path;
  inc(td);
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        Application.ProcessMessages;
        if AddFile(Path + sr.name,ft,ps) = mrCancel then begin Result := mrCancel; goto ex; end;
        if ps >= 0 then inc(ps);
      end;
    end else if (sr.Name<>'.')and(sr.Name<>'..')and(RecursiveDirs) then
    begin
      inc(td);
      if AddDir(Path+sr.Name,ps) = mrCancel then begin Result:= mrCancel; goto ex; end;
      if Assigned(StatusForm) then StatusForm.PathEdit.text := Path;
      Application.ProcessMessages;
    end;
    while (FindNext(sr) = 0)and(not StopProc) do
    begin
      if sr.Attr and faDirectory = 0 then
      begin
        ft := GetFileType(path+sr.name);
    ...
    begin
      ...
      try
        ...
        begin
          ...
          if s = 'FILE' then
          begin
            if not First then AddIt else First := false;
            if tempstr <> '' then
            begin
              if tempstr[2] <> ':' then
                if tempstr[1] = '\' then
...
begin
  ...
  DelOK := true;
  if ConfDiskDel then
  case MessageDlg('Вы действительно хотите Удалить файл(ы) с Диска?',mtWarning,[mbCancel,mbOK],0) of
   mrCancel: exit; 
   mrOK: DelOK := true;
...
begin
  try
    ...
    begin
      ...
      begin
        path := 'Skins\';
//        if not TrySkinPath(path) then Application.Terminate;
      end;
    end;
...
procedure TMainForm.PlayListPTCustomDraw(aSender: TObject;
  aCD: TPTCustomDraw; aItem: TListItem);
begin
  if aCD.DrawStage = ptcdsItemPrePaint then
  begin
    if aItem <> nil then if (aItem.Index = LastPlayed) then aCD.Font.Color := clLime;{}
  end;
end;
