...
type
  TgEdForm = class(TForm)
    ...
    GuessID3v2FromNameBox: TCheckBox;
...
var i : integer;
    ...
    tBand,tTitle,tAlbum,tYear : string; tTrack : integer;
        ...
        begin
          ...
          begin
            ...
            begin
              ...
              begin
                chg := true;
                GetTagFromName(tMPEGAudio(Item[i].Data^).File_Name,tBand,tTitle,tAlbum,tYear,tTrack);
                if tBand <> '' then ID3v1Tag.Artist := copy(tBand,1,30);
                if tTitle <> '' then ID3v1Tag.Title := copy(tTitle,1,30);
                if tAlbum <> '' then ID3v1Tag.Album := copy(tAlbum,1,30);
                if tYear <> '' then ID3v1Tag.Year := copy(tYear,1,4);
                if tTrack > 0 then ID3v1Tag.Track := tTrack;
              end;
              with ID3v1Tag do
              begin
...
{$ENDIF}
          ...
          begin
            ...
            begin
              ...
              if GuessID3v2FromNameBox.Checked then
              begin
                chg := true;
                GetTagFromName(tMPEGAudio(Item[i].Data^).File_Name,tBand,tTitle,tAlbum,tYear,tTrack);
                if tBand <> '' then ID3v1Tag.Artist := copy(tBand,1,30);
              ...
              if T2BandCaseBox.ItemIndex > 0 then
              begin
                if ID3v2Tag.Artist <> '' then chg := true;
                case T2BandCaseBox.ItemIndex of
                 1: ID3v2Tag.Artist := Strcapfirst(ID3v2Tag.Artist);
                 2: ID3v2Tag.Artist := StrCap(ID3v2Tag.Artist);
                 3: ID3v2Tag.Artist := stralldown(ID3v2Tag.Artist);
                 4: ID3v2Tag.Artist := StrAllUp(ID3v2Tag.Artist);
                end;
              end;
              if T2TitleCaseBox.ItemIndex > 0 then
              begin
                if ID3v2Tag.Title <> '' then chg := true;
                case T2TitleCaseBox.ItemIndex of
                 1: ID3v2Tag.Title := Strcapfirst(ID3v2Tag.Title);
                 2: ID3v2Tag.Title := StrCap(ID3v2Tag.Title);
                 3: ID3v2Tag.Title := stralldown(ID3v2Tag.Title);
                 4: ID3v2Tag.Title := StrAllUp(ID3v2Tag.Title);
                end;
              end;
              if T2AlbumCaseBox.ItemIndex > 0 then
              begin
                if ID3v2Tag.Album <> '' then chg := true;
                case T2AlbumCaseBox.ItemIndex of
                 1: ID3v2Tag.Album := Strcapfirst(ID3v2Tag.Album);
                 2: ID3v2Tag.Album := StrCap(ID3v2Tag.Album);
                 3: ID3v2Tag.Album := stralldown(ID3v2Tag.Album);
                 4: ID3v2Tag.Album := StrAllUp(ID3v2Tag.Album);
                end;
              end;

              if T2GenreCaseBox.ItemIndex > 0 then
              begin
                if ID3v2Tag.Genre <> '' then chg := true;
                case T2AlbumCaseBox.ItemIndex of
                 1: ID3v2Tag.Genre := Strcapfirst(ID3v2Tag.Genre);
                 2: ID3v2Tag.Genre := StrCap(ID3v2Tag.Genre);
                 3: ID3v2Tag.Genre := stralldown(ID3v2Tag.Genre);
                 4: ID3v2Tag.Genre := StrAllUp(ID3v2Tag.Genre);
                end;
              end;
