...
//  ArtistEdit.PopupMenu.Items.Add(EdPopUp.Items.Items[1]);
  ...
  begin
    TimeLabel.Caption := TimeSt(mpa.Duration,true);
    GenreBox.Items.Add('');
    iGenreBox.Items.Add('');
    dGenreBox.Items.Add('');
    ...
    begin
      ...
      found := false;
      if (Genre <= MaxGenres) then s := trim(uppercase(Genres[Genre]));
      for i := 0 to GenreBox.Items.Count-1 do
      begin
        t := GenreBox.Items[i];
        if trim(uppercase(t)) = s then
        begin
          Found := true;
          break;
        end;
...
begin
  ...
  s := trim(iTrackEdit.Text);
  if s <> '' then
  begin
    i := 1;
    while (i <= length(s))and(s[i] in ['0'..'9']) do inc(i);
    TrackEdit.Text   := copy(s,1,i-1);
  end;
...
procedure TedForm.Button2Click(Sender: TObject);
var Band,Title,Album,Year : string; Track : integer;
begin
  GetTagFromName(NameEdit.Text,Band,Title,Album,Year,Track);
  if Band <> '' then ArtistEdit.Text := copy(Band,1,30);
  if Title <> '' then TitleEdit.Text := copy(Title,1,30);
  if Album <> '' then AlbumEdit.Text := copy(Album,1,30);
  if Year <> '' then YearEdit.Text := copy(Year,1,4);
  if Track > 0 then TrackEdit.Text := inttostr(Track);
  ID3v1EnabledBox.Checked := true;
end;

procedure TedForm.Button4Click(Sender: TObject);
var Band,Title,Album,Year : string; Track : integer;
begin
  GetTagFromName(NameEdit.Text,Band,Title,Album,Year,Track);
  if Band <> '' then iArtistEdit.Text := Band;
  if Title <> '' then iTitleEdit.Text := Title;
  if Album <> '' then iAlbumEdit.Text := Album;
  if Year <> '' then iYearEdit.Text := Year;
  if Track > 0 then iTrackEdit.Text := inttostr(Track);
  ID3v2EnabledBox.Checked := true;
end;
