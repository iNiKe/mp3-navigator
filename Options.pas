...
//
  ...
  begin
    ...
    begin
      repeat
        if (sr.Attr and faDirectory <> 0)and(sr.Name[1]<>'.')and(uppercase(sr.name) <> 'BASESKIN') then AddSkin(sr.name);
      until FindNext(sr) <> 0;
    end;
    FindClose(sr);
  end;
...
begin
  ...
  sBuf[0]:=#0; SerialNumw := 0;
  SerialNum := 'error loading cr data';
  try
    if GetVolumeInformation(pchar('c:\'), sBuf, dword(SizeOf(sBuf)), @SerialNumw, a, b, nil, 0) then
    begin
      serialnum := IntToStr(SerialNumw);
    end;
  //  pc := ptr($f000 shl 4 + $ec70);
    c1 := 1234;
    c2 := 2345;
    c3 := 3456;
    c4 := 4567;
    SerialNum := 'MPN'+SerialNum;
    pc := pchar(SerialNum);
    for i := 1 to 128 do
    begin
      if pc^ = #0 then break;
      c1 := ((c1 + byte(pc^)) xor i*1);
      c2 := ((c2 + byte(pc^)) xor i*2);
      c3 := ((c3 + byte(pc^)) xor i*3);
      c4 := ((c4 + byte(pc^)) xor i*4);
      inc(pc);
    end;
    c1 := c1 mod 10000;
    c2 := c2 mod 10000;
    c3 := c3 mod 10000;
    c4 := c4 mod 10000;
    SerialNum := inttostr(c1)+'-'+inttostr(c2)+'-'+inttostr(c3)+'-'+inttostr(c4);
    SerialNum := 'M3N'+SerialNum;
  finally
    SetErrorMode(OldErrorMode);
  end;
end.
//#06/23/1998-i440LX-ITE-2A69JL1AC-00
