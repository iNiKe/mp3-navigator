...
//                sz := CalcSize28(id3v2Frame.Size);
                ...
                if ((id3v2Frame.Flags[2] shr 7) and $1 = 0) and
                   ((id3v2Frame.Flags[2] shr 6) and $1 = 0) then
                begin
                  ...
                  begin
                    ...
                    begin
                      val(s,Track,e); if e<>0 then Track := 0;
                    end else
...
*)
                ...
                begin
                  ...
                  begin
                    ...
                    if (ID3v2Tag.isID3v2Tagged){and(ID3v2Tag.Track<>0)} then Track := ID3v2Tag.Track else
...
{          inc(sk,sizeof(tagv2));{}
        ...
        frm.fID := 'TRCK';
        if FData.ID3v2Tag.Track > 0 then s := inttostr(FData.ID3v2Tag.Track) else s := '';
...
//        MakeSize28(length(s) + length(ts) + 2+1,frm.Size);
        frm.Size := length(s) + length(ts) + 2+1;
        BlockWrite(f,frm,sizeof(frm));
        for i := 1 to length(s) do begin BlockWrite(f,s[i],1); end;
        ch := #0;
        BlockWrite(f,ch,1);
...
{ Добавлено (c) 07/01/2002 D.J. NiKe, MP3 NaviGatoR v2.5}
  AddGenres[6]:= 'IDM'; {Intellectual Dance Music}
end;
