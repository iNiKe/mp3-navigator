...
{$ENDIF}
      ...
      appBuildDate = '01/04/2003'; {DD/MM/YYYY}
...
procedure ShowDemoMsg;
begin
  MessageDlg('У Вас Демо-версия программы, сохранение настроек доступно только в полной версии, которую вы можете приобрести по адресу '#13'http://cjnike.h1.ru',mtWarning,[mbok],0);
end;
{$ENDIF}
...
{  FileName := ExpandFileName(FileName);{}
  ...
  try
  begin
    assignfile(tf,filename); reset(tf); nf := 0;
    while (not eof(tf)) do
    begin
      Application.ProcessMessages;
      readln(tf,s);
