...
begin
  ...
  dbpath := basepath+'db\';
  if not fileexists(dbpath+'music.mdb') then
  begin
    dbpath := '\db\';
    if not fileexists(dbpath+'music.mdb') then dbpath := '';
  end;
...
begin
  ...
  begin
    ...
    SongsBox.Text := '';
    if shifted then LyrEdit.Lines.Add('Запрос (Band = "'+rbnd+'" Album = "'+ralb+'" Song = "'+rtit+'" Lyr = "'+rlyr+'"');
